"""
Excel data operations for Excel MCP Server.
"""
...
def read_data_from_excel(
    ...
    Args:
        ...
        range_str: Optional range to read (e.g. 'A1:B10')
...
def write_range(
    ...
    """Write data to a specific range in an Excel workbook.
...
def get_next_available_row(worksheet) -> int:
    ...
    # openpyxl's ws.max_row gives the last row with *any* cell value
    # To be robust, scan from the bottom up for the first row where all cells are empty
    for row in range(worksheet.max_row, 0, -1):
        if any(cell.value is not None for cell in worksheet[row]):
            return row + 1
    return 1

def get_next_available_cell(worksheet) -> str:
    """Return the cell address (e.g., 'A11') for the first cell in the next empty row."""
    next_row = get_next_available_row(worksheet)
    return f"A{next_row}"
